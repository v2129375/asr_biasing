...
keywords = open(f"{train_data_root}/data/weather/lexicon/city.txt").readlines()
...
data = json.load(open(f"{train_data_root}/data/weather/train.json"))
...
for i in data:
    for j in i["utterances"]:
        ...
        path = f"{train_data_root}/data/weather/audios/{j['wav_id']}.wav"
        have_keyword = False
        keyword = ""
        for k in j["semantic"]:
            if k[1] == "城市" and len(k)==3:
                keyword = k[2]
...
for i in data:
    for j in i["utterances"]:
        manual_transcript = find_chinese(j["manual_transcript"]) # 去除非中文的手动转录
        path = f"{test_data_root}/data/weather/audios/{j['wav_id']}.wav"
        have_keyword = False
        keyword = ""
        for k in j["semantic"]:
            if k[1] == "城市" and len(k)==3:
