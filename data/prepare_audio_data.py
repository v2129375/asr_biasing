...
keywords = open(f"{train_data_root}/data/music/lexicon/singer_name.txt").readlines()
...
data = json.load(open(f"{train_data_root}/data/music/train.json"))
...
for i in data:
    for j in i["utterances"]:
        ...
        path = f"{train_data_root}/data/music/audios/{j['wav_id']}.wav"
        have_keyword = False
        keyword = ""
        for k in j["semantic"]:
            if k[1] == "歌手名":
...
# 原先测试集中的内容
data = json.load(open(f"{test_data_root}/data/music/test.json"))
for i in data:
    for j in i["utterances"]:
        manual_transcript = find_chinese(j["manual_transcript"]) # 去除非中文的手动转录
        path = f"{test_data_root}/data/music/audios/{j['wav_id']}.wav"
        have_keyword = False
        keyword = ""
        for k in j["semantic"]:
            if k[1] == "歌手名" and len(k)==3:
                keyword = k[2]
                if not isChinese(keyword): # 不要含有英文的keyword
                    continue
                have_keyword = True
        if have_keyword:
            df = df.append({"text":manual_transcript,"path":path,"keyword":keyword},ignore_index=True)
