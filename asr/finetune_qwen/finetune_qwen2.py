...
USE_BF16 = True  # 使用 bf16 进行微调（需 GPU 计算能力 >= 8.0）
OUTPUT_DIR = 'asr/model/qwen_Keywords3'
BATCH_SIZE = 1
...
def _parse_canonical_user_value(value: str):
    """
    解析范例数据格式中 user 的 value：
    <audio>path</audio> 或 <audio>path</audio><audio>path2</audio>text
    返回 (audio_paths: List[str], text_after_audio: str)
    """
    import re
    audio_paths = re.findall(r"<audio>(.*?)</audio>", value, re.DOTALL)
    text_after_audio = re.sub(r"<audio>.*?</audio>", "", value, flags=re.DOTALL).strip()
    return audio_paths, text_after_audio


def _canonical_to_processor_conversation(conversations: list):
    """
    将范例数据格式 (from/value, value 中含 <audio>path</audio>) 转为
    Qwen2-Audio processor 所需的 role/content + audio_url 格式。
    返回 (conversation_for_processor, transcript)。
    """
    # 取第一条 user 消息解析
    user_msg = next((c for c in conversations if c.get("from") == "user"), None)
    assistant_msg = next((c for c in conversations if c.get("from") == "assistant"), None)
    if not user_msg or not assistant_msg:
        raise ValueError("conversations 需包含 from=user 与 from=assistant 的消息")
    audio_paths, text_after_audio = _parse_canonical_user_value(user_msg["value"])
    transcript = assistant_msg["value"]
    if not audio_paths:
        raise ValueError("user value 中需包含至少一个 <audio>path</audio>")
    content = [{"type": "audio", "audio_url": p} for p in audio_paths]
    if text_after_audio:
        content.append({"type": "text", "text": text_after_audio})
    conversation_for_processor = [{"role": "user", "content": content}]
    return conversation_for_processor, transcript, audio_paths
...
class CatsluKeywordsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 使用范例数据格式: conversations + from/value，user value = "<audio>path</audio>" + 文本
        conversations = [
            {"from": "user", "value": f"<audio>{data['path']}</audio>{instruction}"},
            {"from": "assistant", "value": transcript_label},
        ]
        # 转为 Qwen2-Audio processor 所需格式 (role/content + audio_url)
        conversation_for_processor, _, _ = _canonical_to_processor_conversation(conversations)

        # 应用聊天模板
        prompt = self.processor.apply_chat_template(
            conversation_for_processor, add_generation_prompt=True, tokenize=False
        )
...
def create_model(model_name_or_path, use_flash_attention=False):
    ...
    # 精度：USE_BF16 时使用 bf16（需 GPU 计算能力 >= 8.0），否则 fp16
    major, minor = torch.cuda.get_device_capability(gpu_id)
    supports_bf16 = major >= 8
    if USE_BF16 and supports_bf16:
        torch_dtype = torch.bfloat16
        print("使用 bf16 加载模型")
    else:
        torch_dtype = torch.float16
        if USE_BF16 and not supports_bf16:
            print("当前 GPU 不支持 bf16（需计算能力 >= 8.0），已回退为 fp16")
...
def main():
    ...
    # 混合精度：USE_BF16 时使用 bf16 训练，否则 fp16
    major, minor = torch.cuda.get_device_capability(GPU_IDS[0])
    supports_bf16 = major >= 8
    use_bf16 = USE_BF16 and supports_bf16
    bf16 = use_bf16
    fp16 = not use_bf16
    if use_bf16:
        print("训练使用 bf16 混合精度")
    else:
        print("训练使用 fp16 混合精度")
