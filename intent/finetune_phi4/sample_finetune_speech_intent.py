...
def evaluate(
    ...
    if rank == 0:
        ...
        # 计算分类指标
        accuracy = accuracy_score(all_labels, all_generated_intents)
        f1 = f1_score(all_labels, all_generated_intents, average='weighted')
        report = classification_report(all_labels, all_generated_intents, output_dict=True)
        
        print(f"Accuracy: {accuracy:.4f}")
        print(f"F1 Score: {f1:.4f}")
        print(classification_report(all_labels, all_generated_intents))
